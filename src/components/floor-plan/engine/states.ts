...
export class PanState implements DragState {
	...
	onMove(ctx: ToolContext, e: PointerEvt) {
		...
		const next = {
			...v,
			translateX: this.baseTranslateX + dx,
			translateY: this.baseTranslateY + dy,
		};
...
export class MoveSpaceState implements DragState {
	id = "moveSpace";
	constructor(private startCell: Cell, private spaceId: string) {}
	onMove(ctx: ToolContext, e: PointerEvt) {
    const currentCell = ctx.screenToCell(e.screenX, e.screenY, ctx.store.getState().view);
		const dx = currentCell.x - this.startCell.x;
		const dy = currentCell.y - this.startCell.y;
		ctx.commit(new MoveSpaceCmd   (this.spaceId, dx, dy));
