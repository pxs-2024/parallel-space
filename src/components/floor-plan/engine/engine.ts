import type { FPState, PointerEvt, ToolId, Viewport } from "./types";
import { Store } from "./store";
import { History, Command } from "./history";
import { HitTest } from "./hitTest";
import { screenToCell, screenToPoint } from "./camera";
import { toolById, NoneState } from "./tools";
...
export class FloorPlanEngine {
  ...
  undo() { this.history.undo(this.store); }
  redo() { this.history.redo(this.store); }
  ...
  wheelZoomAt(screenX: number, screenY: number, deltaY: number) {
    ...
    this.store.mutate(s => ({ ...s, view: { translateX: nextTranslateX, translateY: nextTranslateY, scale: nextScale } }));
  }
