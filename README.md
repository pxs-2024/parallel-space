...
- **许可证**：本仓库采用 [MIT License](LICENSE)。使用、复制、修改与分发须遵守该许可条款。
- **贡献**：欢迎通过 Issue 与 Pull Request 参与。提交前请确保通过 `pnpm type` 与项目现有规范；涉及行为准则时请保持尊重与协作。
- **免责**：本产品按「现状」提供，不附带任何明示或暗示保证；使用第三方服务（如 AI 接口）时须自行遵守其服务条款与当地法规。
